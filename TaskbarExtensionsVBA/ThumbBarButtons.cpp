...
// ウィンドウハンドルごとにボタン状態を管理するための構造体を定義
struct WindowButtonState {
    THUMBBUTTON buttons[MAX_BUTTONS] = {};      //ボタン情報格納用
    std::wstring procNames[MAX_BUTTONS];        //コールバック用プロシージャ名の格納用
};
// キー：ウィンドウハンドル(HWND), 値：そのウィンドウのボタン状態
static std::map<HWND, WindowButtonState> g_windowStates;
...
//***************************************************************************************************
static void ExecuteVBAProcByIndex(const std::wstring& procName, HWND targetHwnd) {
    //1.プロシージャ名が空なら、ここで終了
    ...
    CComVariant macroNameArg(procName.c_str());   //引数として渡すのは、VBA側で整形した「'ブック名'!プロシージャ名」
...
static LRESULT CALLBACK SubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
            //このイベントはボタンがクリックされたイベントか判定します(今回は THBN_CLICKED となる)
            if (HIWORD(wParam) == THBN_CLICKED) {
                //補正処理
                int buttonIndex = LOWORD(wParam) - ButtonID_Correction;

                //hwnd をキーにマップからウィンドウ固有の情報を取得
                auto it = g_windowStates.find(hwnd);

                //あったら、次の処理へ
                if (it != g_windowStates.end()) {
                    // マップからプロシージャ名を取得
                    const std::wstring& procName = it->second.procNames[buttonIndex];
                    // プロシージャ名を使ってマクロ実行関数を呼び出す
                    ExecuteVBAProcByIndex(procName, hwnd);
                }
                return 0;
                break;
            }

        //ウィンドウが閉じられる（破棄される）と、Windowsから WM_NCDESTROY という「このウィンドウはもうすぐ完全になくなりますよ」という通知メッセージが送られてきます。
        //これを利用して、ウィンドウが破棄される際のクリーンアップ処理を行います。
        case WM_NCDESTROY:
        {
            // このウィンドウに関連するリソースを解放
            auto it = g_windowStates.find(hwnd);
            if (it != g_windowStates.end()) {
                ...
                for (int i = 0; i < MAX_BUTTONS; ++i) {
                    if (it->second.buttons[i].hIcon) {
                        DestroyIcon(it->second.buttons[i].hIcon);
                    }
                }
                
                // マップからこのウィンドウのエントリを削除
...
void __stdcall InitializeThumbnailButton(HWND hwnd)
{
    ...
    //g_windowStates にこのウィンドウ用のmap(Dictionary)を作成
    auto& state = g_windowStates[hwnd];

    //非表示として、ボタン情報を確保する
    for (int i = 0; i < MAX_BUTTONS; ++i) {
        state.buttons[i].dwMask = THB_FLAGS;
        state.buttons[i].dwFlags = THBF_HIDDEN;
        state.buttons[i].iId = i + ButtonID_Correction;
        state.buttons[i].hIcon = NULL;
        state.buttons[i].szTip[0] = L'\0';
    }

    //反映処理
    g_taskbar->ThumbBarAddButtons(hwnd, MAX_BUTTONS, state.buttons);
...
void __stdcall UpdateThumbnailButton(const THUMBBUTTONDATA* data, const wchar_t* callback)
{
    ...
    //対象ウィンドウhwndの情報をマップから取得
    HWND hwnd = data->TargetHwnd;
    auto it = g_windowStates.find(hwnd);
    if (it == g_windowStates.end()) {
        // 初期化されていないウィンドウに対する更新はここで終了
        return;
    }

    //ボタンIDのオフセット
    int buttonIndex = data->ButtonIndex - ButtonID_Correction;

    //取得したマップから、ボタン情報を取得
    WindowButtonState& state = it->second;
    THUMBBUTTON& btn = state.buttons[buttonIndex];
    ...
    //実行先プロシージャ名
    state.procNames[buttonIndex] = callback ? callback : L"";

    //変更を適用
    g_taskbar->ThumbBarUpdateButtons(data->TargetHwnd, MAX_BUTTONS, state.buttons);
}
